...
LAST.PLAYS <- fread(file.path("raw", "2024", "last_inning_plays.csv"))
...
MERGED.DT <- MERGED.DT[, .(game_id, inning, at_home, outs_post, play_description, player_on_1st, player_on_2nd, player_on_3rd)]
...
MERGED.DT[, batter := str_extract(play_description, "^\\S+")]
MERGED.DT[
  is.na(out_0) & (player_on_1st + player_on_2nd + player_on_3rd == 1L) & 
...
# Export the data
fwrite(MERGED.DT, file.path("raw", "matched_last_inning_plays.csv"))
