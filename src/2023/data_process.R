...
library(stringr)
library(data.table)
DT <- fread(file.path("raw", "2023", "2023plays.csv"))
...
OUTS <- fread(file.path("raw", "2023", "matched_last_inning_plays_w_desc.csv"))
...
# There was one play where there a runner got caught stealing second and then a new batter comes in to finish
# we mark the batter as out because of this batter change but he was not out on the play
stopifnot(nrow(DT[game_id == "BAL202304240" & inning == 7L & !at_home & (out_0 + out_1 + out_2 + out_3 + outs_pre != outs_post)]) == 1L)
DT[
  game_id == "BAL202304240" & inning == 7L & !at_home & (out_0 + out_1 + out_2 + out_3 + outs_pre != outs_post),
  out_0 := FALSE
]

# There are several game-ending plays in which we incorrectly mark the batter was out
stopifnot(nrow(DT[inning >= 9L & at_home & (out_0 + out_1 + out_2 + out_3 + outs_pre != outs_post)]) == 10L)
...
# interference
stopifnot(nrow(DT[walk & error_advance]) == 105L)
...
source(file.path("src", "2023", "manual_updates.R"))
...
fwrite(DT, file.path("processed", "2023_plays.csv"))
