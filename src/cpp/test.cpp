...
DataFrame sim_game_cpp(const ListOf<DataFrame> &POSTERIORs, const DataFrame &RUN_ADV,
  ...
  IntegerVector away_scores(1), home_scores(1);
  ...
  while (inning < 10 || scores[0] == scores[1]) {
    ...
    for (const std::string &team : teams) {
      ...
      Rcout << "  " << (at_home ? "Home" : "Away") << " team batting\n";
      while (n_outs < 3 && !game_over) {
        ...
        if (n_plays[team_idx] > 0.0f) {
          ...
          int next_pitcher_idx =
              pitcher_available(POSTERIOR, pitcher_indices[team_idx], inning, used_pitcher_indices[team_idx]);
          if (next_pitcher_idx != -1) {
            ...
            bool left_batter_incoming =
                lefty_batter_next(POSTERIOR, base_cond, batter_indices[team_idx], pitcher_indices[team_idx]);
            ...
            if (next_pitcher) {
              ...
              Rcout << "    Pitcher change to index: " << next_pitcher_idx << "\n";
            }
          }
        }
        ...
        Rcout << "    Batter " << batter_indices[team_idx] << " vs Pitcher " << pitcher_indices[team_idx]
              << ": " << event_name << "\n";
        ...
        Rcout << "    Score: Away " << scores[0] << ", Home " << scores[1] << ", Outs: " << n_outs << "\n";
  ...
  away_scores[0] = scores[0];
  home_scores[0] = scores[1];
