...
library(stringr)
library(data.table)
DT <- fread(file.path("raw", "2024", "2024plays.csv"))
...
OUTS <- fread(file.path("raw", "2024", "matched_last_inning_plays_w_desc.csv"))
...
# There was one play where the base runner stole home on a wild pitch to end the game where we mark the player
# out but should not be ruling him out
stopifnot(nrow(DT[game_id == "SFN202407090" & inning == 9L & at_home & batter == "estrt001" & wild_pitch]) == 1L)
DT[game_id == "SFN202407090" & inning == 9L & at_home & batter == "estrt001" & wild_pitch, out_0 := FALSE]
...
# interference
stopifnot(nrow(DT[walk & error_advance]) == 106L)
...
source(file.path("src", "2024", "manual_updates.R"))
...
fwrite(DT, file.path("processed", "2024_plays.csv"))
