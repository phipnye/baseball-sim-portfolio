...
library(stringr)
library(data.table)
DT <- fread(file.path("raw", "2021", "2021plays.csv"))
...
OUTS <- fread(file.path("raw", "2021", "matched_last_inning_plays_w_desc.csv"))
...
# There are three plays in which the batter was subbed out during the PA and we erroneously mark them as out
# since they aren't on base
stopifnot(nrow(DT[out_0 & outs_pre == outs_post]) == 3L)
DT[out_0 & outs_pre == outs_post, out_0 := FALSE]
...
# interference
stopifnot(nrow(DT[walk & error_advance]) == 68L)
...
source(file.path("src", "2021", "manual_updates.R"))
...
fwrite(DT, file.path("processed", "2021_plays.csv"))
